...
@section('title', 'Weekend | Ad Packages')
...
<div class="d-flex align-items-center justify-content-between mb-3">
    <div>
        <h4 class="fw-bold mb-1">{{ __('lang.ad_packages') }}</h4>
        <div class="text-muted">{{ __('lang.manage_ad_subscription_packages') }}</div>
    </div>

    @if($me && $me->can('ad_packages.create'))
        <button class="btn btn-accent" data-bs-toggle="modal" data-bs-target="#createPackageModal">
            + {{ __('lang.create_ad_package') }}
        </button>
    @endif
</div>
            ...
            <table class="table align-middle">
                ...
                @forelse($packages as $p)
                    <tr>
                        ...
                        <td class="text-end">
                            @if($me && $me->can('ad_packages.view'))
                                <a href="{{ route('admin.ad-packages.show', $p->id) }}" class="btn btn-sm btn-outline-secondary">
                                    {{ __('lang.view') }}
                                </a>
                            @endif
                            ...
                            @if($me && $me->can('ad_packages.delete'))
                                <form action="{{ route('admin.ad-packages.destroy', $p->id) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
...
<div class="modal fade" id="createPackageModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form class="modal-content" method="POST" action="{{ route('admin.ad-packages.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="modal-header">
                <h5 class="modal-title">{{ __('lang.create_ad_package') }}</h5>
...
<div class="modal fade" id="editPackageModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form class="modal-content" method="POST" id="editPackageForm" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="modal-header">
                <h5 class="modal-title">{{ __('lang.edit_ad_package') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
@push('js')
<script>
function togglePackageTypeFields(typeId, countId, durationId) {
    const typeEl = document.getElementById(typeId);
    const countEl = document.getElementById(countId);
    const durationEl = document.getElementById(durationId);

    if (!typeEl || !countEl || !durationEl) return;
...
document.getElementById('createPackageType')?.addEventListener('change', function () {
    togglePackageTypeFields('createPackageType', 'createPackageCount', 'createPackageDuration');
});
togglePackageTypeFields('createPackageType', 'createPackageCount', 'createPackageDuration');

document.getElementById('editPackageType')?.addEventListener('change', function () {
    togglePackageTypeFields('editPackageType', 'editPackageCount', 'editPackageDuration');
});
...
document.getElementById('editPackageModal')?.addEventListener('show.bs.modal', function (event) {
    ...
    document.getElementById('editPackageForm').action = `{{ url('/admin/ad-packages') }}/${id}`;
    ...
    document.getElementById('editPackageType').value = btn.getAttribute('data-type') || 'count';
